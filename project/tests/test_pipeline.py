...
def load_dataframe(db_path, table_name):
    ...
    df = pd.read_sql_query(f"SELECT * FROM {table_name}", connection_db)
    connection_db.close()
    return df
...
@pytest.fixture
def fect_total_df():
    database_path = 'data/energy_consumption.db'
    table_name = 'FECT_Total'
    return load_dataframe(database_path, table_name)
...
def test_database_file_exists():
    ...
    assert os.path.exists(
        database_path), "The SQLite database file does not exist"
...
def test_fect_total_table_exists():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='FECT_Total';")
...
def test_fect_road_table_exists():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='FECT_Road';")
    result = cursor.fetchone()
    conn.close()
...
def test_fect_total_schema(fect_total_df):
    ...
    assert list(
        fect_total_df.columns) == expected_columns, "FECT_Total table schema does not match the expected schema"
...
def test_fect_road_schema(fect_road_df):
    ...
    assert list(
        fect_road_df.columns) == expected_columns, "FECT_Road table schema does not match the expected schema"
...
def test_convert_to_datetime():
    ...
    assert pd.api.types.is_datetime64_any_dtype(
        df['TIME_PERIOD']), "Column 'TIME_PERIOD' was not converted to datetime"
